...
function church_admin_hope_team_jobs()
{
	...
	{	
		...
		$out.='<table class="widefat striped"><thead><tr><th>'.__('Edit','church-admin').'</th><th>'.__('Delete','church-admin').'</th><th>'.__('Job Title','church-admin').'</th></tr></thead><tbody>';
		foreach($results AS $row)
		{
		$edit='<a href="'.wp_nonce_url('admin.php?page=church_admin/index.php&amp;action=edit_hope_team_job&amp;id='.intval($row->hope_team_id),'hope_team_jobs').'">'.__('Edit','church-admin').'</a>';
		$delete='<a  onclick="return confirm(\''.__('Are you sure?','church-admin').'\');" href="'.wp_nonce_url('admin.php?page=church_admin/index.php&amp;action=delete_hope_team_job&amp;id='.intval($row->hope_team_id),'delete_hope_team_job').'">'.__('Delete','church-admin').'</a>';
...
function church_admin_delete_hope_team_job($id=NULL)
{
	...
	echo'<div class="notice notice-success inline">Job Deleted</div>';
...
function church_admin_edit_hope_team_job($id=NULL)
{
	...
	if(!empty($_POST['save_job']))
	{
		...
		echo'<div class="notice notice-success inline">'.__('Job saved','church-admin').'</div>';
		church_admin_hope_team_jobs();
	}
	else
	{
		...
		echo'/></p>';
		 echo'<p class="submit"><input type="hidden" name="save_job" value="yes"/><input type="submit" class="button-primary" name="save" value="'.__('Save','church-admin').' &raquo;" /></p></form></div>';
	}
		...
		if(!empty($people_ids))
		{
			foreach($people_ids AS $person_id)
			{
				...
				foreach($jobs AS $job)
				{
					...
					{
						$sql='INSERT INTO '.CA_MET_TBL.' (meta_type,people_id,ID)VALUES("hope_team","'.esc_sql($person_id).'","'.esc_sql($job->hope_team_id).'")';
			...
			echo'<div class="notice notice-success inline"><p><strong>Hope Team edits saved</strong></p></div>';
		}
