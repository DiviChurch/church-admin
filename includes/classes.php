...
function  church_admin_classes()
{
	...
	if(!empty($classes))
	{
		...
		foreach($classes AS $row)
		{
            ...
            $delete='<a onclick="return confirm(\''.__('Are you sure?','church-admin').'\');" href="'.wp_nonce_url('admin.php?page=church_admin/index.php&amp;tab=people&amp;action=delete_class&amp;id='.intval($row->class_id),'delete_class').'">'.__('Delete','church-admin').'</a>';
            echo'<tr>';
...
function church_admin_delete_class($class_id=NULL)
{
	...
	echo'<div class="notice notice-success inline"><p>'.__('Class Deleted','church-admin').'</p></div>';
...
function church_admin_edit_class($class_id=NULL)
{
	...
	if(!empty($_POST['save']))
	{
		...
		if(empty($class_id))
		{
			$query='INSERT INTO '.CA_CLA_TBL.' (name,description,next_start_date,recurring,how_many,start_time,end_time)VALUES("'.$sql['name'].'","'.$sql['description'].'","'.$sql['next_start_date'].'","'.$sql['recurring'].'","'.$sql['how_many'].'","'.$sql['start_time'].'","'.$sql['end_time'].'")';
			
			$wpdb->query($query);
			$class_id=$wpdb->insert_id;
		}
		else{
			$query='UPDATE '.CA_CLA_TBL.' SET name="'.$sql['name'].'" , description="'.$sql['description'].'" , next_start_date="'.$sql['next_start_date'].'" , recurring="'.$sql['recurring'].'" , how_many="'.$sql['how_many'].'" AND start_time="'.$sql['start_time'].'" AND end_time="'.$sql['end_time'].'" WHERE class_id="'.esc_sql($class_id).'"';
			$wpdb->query($query);
		}
	...
	else
	{
		...
		echo'<tr><th scope="row">'.__('Next Start Date','church-admin').' (yyyy-mm-dd)</th><td>'.church_admin_date_picker($current,'next_start_date',FALSE,date('Y'),date('Y')+10).'</td></tr>';
...
function church_admin_view_class($id)
{
	...
	if(!empty($_POST['date'])&&checkDateFormat($_POST['date'])){$date=esc_sql($_POST['date']);}else{$date=date('Y-m-d');}
		...
		if(!empty($complete))
		{
			...
			$wpdb->query('INSERT INTO '.CA_MET_TBL.' (meta_type,ID,people_id,meta_date)VALUES '.implode(',',$values));
		}
	...
	if(!empty($_POST['delegate']))
	{
		...
		if(!empty($delegate))
		{
			...
			$sql='INSERT INTO '.CA_MET_TBL.' (meta_type,ID,people_id,meta_date)VALUES '.implode(',',$values);
	...
	echo'<tr><td colspan=2><input type="submit" class="button-primary" value="'.__('Save','church-admin').'"/></td></tr>';
	echo'</table></form>';
	$people=$wpdb->get_results('SELECT CONCAT_WS(" ",a.first_name,a.last_name) AS name,b.meta_date FROM '.CA_PEO_TBL.' a, '.CA_MET_TBL.' b WHERE a.people_id=b.people_id AND b.meta_type="class" AND b.ID="'.esc_sql($id).'"');
	if(!empty($people))
	{
	...
	church_admin_show_comments('classes',$id);
}
?>