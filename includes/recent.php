	...
	if(!empty($results))
	{
		...
		foreach($recent_dates AS $service_date=>$people)
		{
			...
			foreach($people AS $person)
			{
				...
				$out.='<p><a title="Edit person" href="'.wp_nonce_url('admin.php?page=church-admin/index.php&amp;action=edit_people&amp;people_id='.intval($person['people_id']),'edit_people').'">'.esc_html($person['first_name']).'</a> <a href="'.wp_nonce_url('admin.php?page=church-admin/index.php&amp;action=edit_household&amp;household_id='.intval($person['household_id']),'edit_household').'" title="edit household">'.esc_html($person['last_name']).'</a></p>';
