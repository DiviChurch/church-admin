...
function church_admin_mailchimp_sync()
{
	...
	{
		echo'<div class="notice notice-success inline">Processing</div>';
		...
		echo'<div class="notice notice-success inline">Mailchimp groups created/updated</div>';
		...
		if(!empty($people))
		{
			...
			foreach($people AS $person)
			{
				...
				//grab ministries
				$result=$wpdb->get_results('SELECT ID FROM '.CA_MET_TBL.' WHERE people_id="'.$person->people_id.'" AND meta_type="ministry"');
				$mins=$min=array();
				if(!empty($result))
				{
						foreach($result AS $row)
						{
							if($row->ID!=0)$min[]=$ministries[$row->ID];
							
						}
				}
				if(!empty($min))$mins=array('id'=>$ministry_id,'groups'=>$min);
				$result=$wpdb->get_results('SELECT ID FROM '.CA_MET_TBL.' WHERE people_id="'.$person->people_id.'" AND meta_type="hope_team"');
				$htjobs=$ht_jobs=array();
				if(!empty($result))
				{
						foreach($result AS $row)
						{
							if($row->ID!=0)$ht_jobs[]=$hope_teams[$row->ID];
		...
		echo'<div class="notice notice-success inline">Mailchimp subscribers created/updated</div>';
