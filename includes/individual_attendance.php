 ...
 */
	...
	{//process form
		...
		$meeting=explode('-',sanitize_text_field($_POST['meeting']));
		$sqlsafe=array();
		$adult=$child=0;
		...
		$sql='INSERT INTO '.CA_ATT_TBL .' (`date`,adults,children,service_id,mtg_type)VALUES("'.$sqlsafe['date'].'","'.$adult.'","'.$child.'","'.$meeting[1].'","service")';
		
		if($meeting[0]=='service')$wpdb->query($sql);
		echo'<div class="notice notice-success inline"><p>'.__('Attendance Updated','church-admin').'</p></div>';
	}//end process form
	//form
