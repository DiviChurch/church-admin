...
function church_admin_new_calendar($current=NULL,$facilities_id=NULL)
{
	...
	if(!empty($facilities_id))
	{
		...
		$out.='<h2>'.__('Bookings Calendar for ','church-admin').$facility.'</h2>';
	}else
	{
		$out.='<h2>'.__('Calendar','church-admin').'</h2>';
	}
	$out.='<p><a href="admin.php?page=church_admin/index.php&amp;action=church_admin_category_list">'.__('Category List','church-admin').'</a></p>';
	
	$out.='<p>'.__('Double click on an event to edit, or a day to add an event','church-admin').'</p>';
	$out.='<p><a href="'.admin_url().'?page=church_admin/index.php&action=church_admin_calendar_list&tab=calendar">'.__('Old Style Calendar List','church-admin').'</a></p>';
	...
	if(!empty($facilities_id)) $out.='<input type="hidden" name="facilities_id" value="'.intval($facilities_id).'"/>';
	$out.='<input type="submit" class="calendar-date-switcher" value="'.__('Next','church-admin').'"/></form></td></tr>
    ...
    else
    {
        foreach($result AS $row)
        {
    		...
    		{//all day
    			$out.= '<div id="item'.$row->date_id.'"style="background-color:'.$row->bgcolor.';border-left:3px solid '.$border.';padding:5px;color:'.$text.'" >'.__('All Day','church-admin').' '.esc_html($row->title).'... </div></p>';

    		}
    		else
    		{
				$out.= '<div id="item'.$row->date_id.'"style="background-color:'.$row->bgcolor.';border-left:3px solid '.$border.';padding:5px;color:'.$text.'" >'.mysql2date(get_option('time_format'),$row->start_time).' '.esc_html($row->title).'... </div></p>';
